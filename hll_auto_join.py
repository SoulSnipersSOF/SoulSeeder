...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    ...
    def get_server_player_count(self, server_key):
        """Get actual player count from Battlemetrics API"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_status(f"📊 {server_name}: {player_count} players")
                return player_count
            else:
                self.log_status(f"⚠️ API Error {response.status_code} for {server_key}")
                return 0

        except Exception as e:
            self.log_status(f"❌ Error getting player count for {server_key}: {str(e)}")
            return 0
    ...
    def launch_hll_with_server(self, server_info):
        """Launch HLL with specific server connection (for manual joins)"""
        server_connection = f"-dev +connect {server_info['ip']}:{server_info['port']}"

        # Use the enhanced backup and append method
        if self.modify_steam_config_with_backup(server_connection):
            try:
                steam_exe = os.path.join(self.steam_path, "steam.exe")

                self.log_status("🚀 Launching Hell Let Loose...")
                subprocess.Popen([steam_exe, "-applaunch", self.HLL_APP_ID])

                self.log_status(f"✅ Connecting to {server_info['name']}")

                # The automatic cleanup is now handled by modify_steam_config_with_backup
                # No need to schedule it again here

            except Exception as e:
                self.log_status(f"❌ Error launching game: {str(e)}")
                messagebox.showerror("Error", f"Failed to launch game: {str(e)}")
        else:
            messagebox.showerror("Error", "Failed to configure game for automatic connection")
    ...
    def run_seeding_loop(self):
        """Main seeding loop - runs in separate thread"""
        try:
            self.log_status("📊 Checking server player counts...")
            soul1_players = self.get_server_player_count('soul1')
            soul2_players = self.get_server_player_count('soul2')
            ...
            # FIXED: Only launch for the TARGET server, not both
            ...
            time.sleep(90)  # Increased from 80 seconds
            ...
            self.monitoring_loop(target_server)
    ...
    def launch_hll_with_server_monitoring(self, server_info):
        """Launch HLL with server connection for monitoring (for auto-seeding)"""
        server_connection = f"-dev +connect {server_info['ip']}:{server_info['port']}"
    ...
    def monitoring_loop(self, current_server):
        """Enhanced continuous monitoring loop with server switching logic"""
        while self.monitoring_active:
            try:
                ...
                self.root.after(0, lambda: self.selected_label.config(
                    text=f"Seeding {self.SERVERS[current_server]['name']} | Soul 1: {soul1_players} | Soul 2: {soul2_players}",
                    fg='#4caf50'
                ))

                # Check if both servers are full - mission accomplished!
                if soul1_players >= 70 and soul2_players >= 70:
                    ...
                    # Clear Steam options and exit cleanly
                    self.root.after(0, self.clear_steam_launch_options_and_exit)
                    break

                # Server switching logic
                if current_server == 'soul1' and soul1_players >= 70:
                    ...
                    if soul2_players < 70:
                        ...
                        # Close HLL and Steam completely for clean server switch
                        self.log_status("🚪 Closing HLL and Steam for server transition...")
                        ...
                        if self.launch_hll_with_server_monitoring(server_info):
                            ...
                            continue  # Continue monitoring Soul 2
                        else:
                ...
                elif current_server == 'soul2' and soul2_players >= 70:
                    self.log_status(f"✅ Soul 2 is full with {soul2_players} players!")
                    self.log_status("🎉 Both servers are now populated! Seeding mission complete!")
                    self.root.after(0, self.clear_steam_launch_options_and_exit)
                    break

                # Log current status periodically
                if int(time.time()) % 180 == 0:  # Every 3 minutes
                    self.log_status(f"📊 Monitoring: Soul 1 ({soul1_players}) | Soul 2 ({soul2_players}) | Current: {self.SERVERS[current_server]['name']}")
